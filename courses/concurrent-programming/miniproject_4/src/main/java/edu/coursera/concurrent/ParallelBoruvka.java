 ...
 * Spanning Tree.
 */
public final class ParallelBoruvka extends AbstractBoruvka<ParallelBoruvka.ParallelComponent> {

    /**
     * Constructor.
     */
    public ParallelBoruvka() {
        super();
    }
     ...
     */
    @Override
    public void computeBoruvka(final Queue<ParallelComponent> nodesLoaded, final SolutionToBoruvka<ParallelComponent> solution) {
        ParallelComponent component;
        while ((component = nodesLoaded.poll()) != null) {
            ...
            Edge<ParallelComponent> minEdge = component.getMinEdge();
            if(minEdge == null)
                break;

            ParallelComponent other = minEdge.getOther(component);
     ...
     */
    public static final class ParallelComponent extends Component<ParallelComponent> {
         ...
         * to greatest.
         */
        private List<Edge<ParallelComponent>> edges = new ArrayList<>();
         ...
         */
        public ParallelComponent(final int setNodeId) {
            super();
         ...
         */
        public void addEdge(final Edge<ParallelComponent> e) {
            int i = 0;
            while (i < edges.size()) {
         ...
         */
        public Edge<ParallelComponent> getMinEdge() {
            if (edges.size() == 0) {
                return null;
            }
            return edges.get(0);
         ...
         */
        public void merge(final ParallelComponent other, final double edgeWeight) {
            totalWeight += other.totalWeight + edgeWeight;
            totalEdges += other.totalEdges + 1;

            final List<Edge<ParallelComponent>> newEdges = new ArrayList<>();
            int i = 0;
            int j = 0;
            while (i + j < edges.size() + other.edges.size()) {
                // Get rid of inter-component edges
                while (i < edges.size()) {
                    final Edge<ParallelComponent> e = edges.get(i);
                ...
                while (j < other.edges.size()) {
                    final Edge<ParallelComponent> e = other.edges.get(j);
                    if ((e.fromComponent() != this && e.fromComponent() != other)
                            || (e.toComponent() != this && e.toComponent() != other)) {
                        break;
                    }
     ...
     */
    public static final class ParEdge extends Edge<ParallelComponent> implements Comparable<Edge> {
        /**
         * Source component.
         */
        protected ParallelComponent fromComponent;

        /**
         * Destination component.
         */
        protected ParallelComponent toComponent;
         ...
         */
        public ParEdge(final ParallelComponent from, final ParallelComponent to, final double w) {
            fromComponent = from;
            toComponent = to;
            weight = w;
        }
         ...
         */
        @Override
        public ParallelComponent fromComponent() {
         ...
         */
        @Override
        public ParallelComponent toComponent() {
            return toComponent;
        }
         ...
         * {@inheritDoc}
         */
        public ParallelComponent getOther(final ParallelComponent from) {
         ...
         * {@inheritDoc}
         */
        public ParEdge replaceComponent(final ParallelComponent from, final ParallelComponent to) {
            if (fromComponent == from) {
                fromComponent = to;
            }
            if (toComponent == from) {
